...
An example data set and data dictionary are included in every {soils} project. You can access these `.csv` files in the `data` folder or by loading the package with `library(soils)`. Use these examples as templates to correctly format your own data and test them in other {soils} functions.
...
All measurement result columns should come **after** the required columns to make it easier to select in the data wrangling code.
...
-   `column_name` is the join key for joining with your project data. Must be exactly the same as the column names for each measurement.
-   `abbr` and `unit` are how the measurements are represented in `flextable` tables.
-   `abbr_unit` is formatted with HTML line breaks and special characters for `ggplot2` plots.
