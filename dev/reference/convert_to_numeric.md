...
- data:
  ...
  A data frame containing both metadata and quantitative measurement
  columns.
...
#> Warning: ℹ Non-numeric values were converted to `NA`. (e.g., `ND` or `<1`). These values
#>   are excluded from tables and plots.
#> 
#> ! Columns with values converted to `NA`:
#> • ph (1 value)
#> • nh4_n_mg_kg (2 values)
#> • no3_n_mg_kg (3 values)
```
