...
subtitle: "Fall 2023"
# EDIT: A single producer_id and year that is a valid combo that exists in your dataset.
params:
  producer_id: WUY05
  year: 2023
...
# OPTIONAL EDIT: rearrange order of measurement_groups in the below vector. This
# vector determines the order that the measurement groups appear in the results
# section of the report. Each measurement group must be included in the
# dictionary.
...
```{r data-validation}
# Check that data contains the required columns and there is a column_name entry
# in the data dictionary for each measurement column.
testthat::expect_in(
  names(data),
  c(
    "year",
...
testthat::expect_in(dictionary$measurement_group, measurement_groups)
```
...
```{r tidy-long}
# EDIT: Replace the below indices with the column range containing soil health
# measurement results in your own dataset. For example, washi_data.csv has
# measurement results in columns 13 - 43.

# Tidy data into long format and join with data dictionary
results_long <- data |>
  dplyr::mutate(
    dplyr::across(
      # EDIT: replace with the column range of your measurement results
      13:43,
      as.numeric
    )
  ) |>
...
data$field_name <- ifelse(
  is.na(data$field_name),
  data$field_id,
  data$field_name
)

# If farm name is blank, use producer ID
data$farm_name <- ifelse(
  is.na(data$farm_name),
  data$producer_id,
  data$farm_name
)
...
# In case there is no farm name, use producer ID
producer$farm_name <- ifelse(is.na(producer$farm_name),
