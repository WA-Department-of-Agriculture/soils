...
-   **`field_id <chr>`** must be unique across all fields for each producer and
    is displayed in tables and tool tips for maps.
...
-   `longitude <int>` and `latitude <int>` are used to map each sample point
    using. If blank, the map will not be included.
...
-   `texture <chr>` is used in the “physical” measurement group table. This
    ...
    from percentage sand, silt, and clay measurement results. If at least two of
    `sand_percent`, `silt_percent`, and `clay_percent` are provided, `texture`
    will be classified according to the USDA NRCS textural classification. See
    `classify_texture()` for more information.
...
Ensure the column ranges in `dplyr::across()` and `tidyr::pivot_longer()` match
the column range of your soil measurements. In the example `washi-data.csv`,
this range is `11:41`. This chunk [tidies the
data](https://vita.had.co.nz/papers/tidy-data.pdf) from wide to long for
summarization and visualization.

In this example, imagine you have one additional metadata column before your
measurement columns begin. This means your column range of soil measurements is
`12:42`.
