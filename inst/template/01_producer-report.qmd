...
assertr::assert(
  ...
  measurement_group,
  description = "`dictionary` contains measurement group\\
                that isn't defined."
)
```
...
# results in your own dataset.
# 
# For example, the results in `washi_data` are located in columns 11 through 49.

# Tidy data into long format and join with data dictionary
results_long <- data |>
  dplyr::mutate(
    dplyr::across(
      # EDIT: column index range
      11:49,
      as.numeric
    )
  ) |>
...
## `r ifelse(!is.na(producer$farm_name), producer$farm_name, paste("Farm:", params$producer_id))`
