...
# Load lab results
data <- read.csv(
  here::here("inst/extdata/example_data_wide.csv"),
  check.names = FALSE
)

# Load data dictionary for pretty labels
dictionary <- read.csv(
  here::here("inst/extdata/data_dictionary.csv"),
  encoding = "UTF-8"
)
...
invisible(htmlwidgets::saveWidget(
  map,
  file = here::here("inst/figure_output/map.html")
))
invisible(webshot::webshot(
  url = here::here("inst/figure_output/map.html"),
  file = here::here("inst/figure_output/map.png"),
))
```
...
{{< include _looking_forward.qmd >}}

{{< include _acknowledgement.qmd >}}
