...
-   **`producer_id <chr>`** must be unique throughout dataset and is used to
    select samples to be included in the report.
...
without errors will likely be the most difficult part of customizing the {soils}
reports. The [`data-validation` chunk] includes `testthat` functions to fail
early if required columns are missing from either the data or the dictionary.
...
data with the `data <- dplyr::select(data, -c("cec_meq_100g", "phone_number"))`.
If you want to use this column in the report, then add it to the `required_cols`
