...
Note for Mac OS users: facetted `{plotly}` strip plots have [issues](https://github.com/plotly/plotly.R/issues/1224) with overlapping labels. The default `panel_spacing` arguments in `make_plotly()` are optimized for rendering reports on a Windows machine. If the plots don't look right on your machine, try modifying the `panel_spacing` arguments as described in the function [documentation](https://wa-department-of-agriculture.github.io/soils/reference/make_plotly.html).
